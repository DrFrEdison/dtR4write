produkt_per_day_year <- function(customer, location, line, LG, year, date_file = NA, dir_wd){
  ...
  for(k in 1:length(filep$year)){
    ...
    for(j in 1:length(filep$files)){
      ...
      if(LG != "3" & LG != "SG3"){
        filep$export <- data.frame(cbind(unique( filep$trans[ , get("date")]), unique(filep$trans[ , grep("Produkt", names(filep$trans)), with = F])))
      ...
      if(LG == "3" |  LG == "SG3"){
        ...
        if( length( grep("DTproductNumber", names(filep$trans))) > 0 &
            length( grep("MixerNumber", names(filep$trans))) == 0) filep$export <- data.frame(date = unique( filep$trans[ , get("date")])
                                                                                              , Produktname = NA
      ...
      #
      #   file.copy( paste0(wd$data, "/dt_Mixernummer.ods")
      #              , paste0(wd$data, "/", date.dt(), "_", "dt_Mixernummer.ods"))
      #
      ...
      #                      , paste0(wd$data, "/", substr(gsub("-", "", Sys.Date()), 3, 8), "_dt_Mixernummer.ods"))
      #
      # }
